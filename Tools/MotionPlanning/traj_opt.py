...
def traj_opt_penalty_sqp(X, U, model, plotting=False, profile=False):
    cfg = model.sqpParams
    X_G = X + np.zeros(X.shape)
    ...
    while iterCount <= cfg.max_penalty_coeff_increases and not success:    
        ...
        [X, U, success] = minimize_merit_function(X,U,X_G,model, cfg, penalty_coeff, trust_box_size, plotting, profiler)
	
        if plotting:
            plot.plot_belief_trajectory(X, U, model);
    
        success = constraints_satisfied(X, U, model, cfg.cnt_tolerance)
...
def compute_merit(X, U,X_G, model, penalty_coeff):
    xDim = model.xDim
    T = model.T
    merit = 0
    for t in xrange(0,T-1):
        merit += model.alpha_belief*ml.sum(np.abs(X[0:2,t] - X_G[0:2,t]))
        merit += model.alpha_control*ml.sum(U[:,t].T*U[:,t])
      
        merit += penalty_coeff*ml.sum(np.abs(X[:,t+1]-belief.belief_dynamics(X[:,t],U[:,t],None,model)))
    
    merit += model.alpha_final_belief*ml.sum(np.abs(X[0:2,T-1]-X_G[0:2,T-1]))
    return merit

# Same as compute_merit, but linearized version for use with optimization
def linearized_compute_merit(B, Xcvx,X_G, U, Ucvx, gval, G, H, model, penalty_coeff):
    xDim = model.xDim
    uDim = model.uDim
    T = model.T
    ...
    for t in xrange(0,T-1):
        trace_merits.append(model.alpha_belief*cvxpy.sum_entries(cvxpy.abs(Xcvx[0:2,t]-X_G[0:2,t])))

        control_merits.append(model.alpha_control*cvxpy.sum_entries(cvxpy.square(Ucvx[:,t])))
        belief_penalty_merits.append(penalty_coeff*cvxpy.sum_entries(cvxpy.abs(Xcvx[:,t+1] - (gval[t] + G[t]*(Xcvx[:,t]-B[:,t]) + H[t]*(Ucvx[:,t]-U[:,t])))))
    
    trace_merits.append(model.alpha_final_belief*cvxpy.sum_entries(cvxpy.abs(Xcvx[0:2,T-1]-X_G[0:2,T-1])))
...
def minimize_merit_function(B, U,B_G, model, cfg, penalty_coeff, trust_box_size, plotting=True, profiler=None):
    success = True
    sqp_iter = 1
    ...
    while True:
        ...
        merit = compute_merit(B, U,B_G, model, penalty_coeff);
        #print('merit: %f' % merit)
        ...
        while True:
            ...
            objective_merit = linearized_compute_merit(B,Bcvx,B_G,U,Ucvx,gval,G,H,model,penalty_coeff)
	    ...
	    try:
                cvx_optval = problem.solve(verbose=True,solver='CVXOPT')
            ...
            new_merit = compute_merit(Bcvx, Ucvx,B_G, model, penalty_coeff)
...
def constraints_satisfied(B, U, model, tolerance):
	...
	done &= np.max(B[0:xDim,t] <= xMax)
	done &= np.min(B[0:xDim,t] >= xMin)
